        ...
        <div class="bg-white rounded-lg shadow-lg max-w-sm mt-4">
            <div class="flex justify-center">
              <div class="relative">
                ...
                <img class="w-24 h-24 rounded-full border-4 border-white absolute top-0" src="{{ url('/data_file/'.$user->picture) }}" alt="Profile image">
                @else
                <img class="w-24 h-24 rounded-full border-4 border-white absolute top-0" src="{{asset('images/it.png')}}" alt="Profile image">
                @endif
              </div>
            </div>
              ...
              @if (count($workExperiences))
                ...
                <div class="flex items-start mb-4 border-2 border-black rounded p-5">
                  <div class="flex-shrink-0">
                      <img src="{{asset('images/sales.png')}}" alt="UX UI Designer" class="w-8 h-8 rounded-full mr-2"> <!-- Image on the left -->
                  </div>
                  <div class="flex-grow">
                      <div class="flex items-center justify-between">
                          <h3 class="text-lg font-semibold">{{ $w->position }}</h3>
                          <span class="text-sm text-gray-500">{{ $w->start_year  }} - {{ $w->end_year  }}</span>
                      </div>
                      <div class="text-sm text-gray-500 mb-2">
                          <span class="font-medium">🔁 Fulltime</span> |📍 <span>{{ $w->company  }}</span>
                      </div>
          ...
          <div class="mb-4">
            ...
            <div class="flex items-start mb-4 border-2 border-black rounded p-5">
              ...
              <div class="flex-grow">
                  ...
                  <div class="text-sm text-gray-500 mb-2">
                      <span class="font-medium">📅 {{ $e->start_year }} - {{ $e->end_year }}</span>
                  </div>
              </div>
          </div>
          @endforeach
